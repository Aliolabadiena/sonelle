...
luna's healing is a loop: a script **detects**, Claude **fixes**. No magic — a tight,
...
1. **Detect** — run `tools\doctor.ps1 [<short>]`. It checks:
   ...
   - each project's own checks via the hook `<code path>\luna.check.ps1` (exit 0 = healthy).
...
## Define a project's health (`luna.check.ps1`)
Put real checks in `<code path>\luna.check.ps1` — whatever proves the project is healthy.
